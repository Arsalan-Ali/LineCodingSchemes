...
public class B_PseudoView extends CommonView {

private DataCentre Data;
	
	public B_PseudoView(Context context) {
		super(context);
		initSigPaint();
	}
	
	public B_PseudoView(Context context, AttributeSet attrs) {
		super(context, attrs);
		initSigPaint();
	}
	public B_PseudoView(Context context, AttributeSet attrs,int defStyle) {
		...
		for(int i=0;i<Data.getBits().length();i++)
		{
			...
			if(Data.getBits().charAt(i)=='1'){
				...
				if(Data.getBits().length()>(i+1))
					...
					else if(Data.getBits().charAt(i+1)=='0'){
						vertLnUp(canvas,i+1);
						vertLnDown(canvas,i+1);
					}
				drawBit(canvas,'0',i);
